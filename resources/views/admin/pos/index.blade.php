                ...
                <div x-show="orderData && !loading" style="display: none;" class="flex-1 flex flex-col relative z-10">
                    ...
                    <div class="pt-5 border-t border-dashed border-neutral-200 mt-auto">
                        ...
                        <div class="flex space-x-3">
                            ...
                            <!-- Nanti bisa disambungkan dengan fungsi update status ke "completed" -->
                            <form :action="'/admin/orders/' + orderData?.id + '/status'" method="POST" class="flex-1" x-show="orderData?.status === 'pending' || orderData?.status === 'cooking' || orderData?.status === 'ready' || orderData?.status === 'confirmed'">
                                @csrf
                                <input type="hidden" name="status" value="completed">
                                <button type="submit" class="w-full py-3.5 px-4 bg-[#7A1F2B] hover:bg-[#5a1620] text-white font-bold rounded-xl shadow-lg transition-all text-sm flex items-center justify-center space-x-2">
